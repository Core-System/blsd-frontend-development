...
const links = [
  { rotulo: 'Início', rota: '/' },
  { rotulo: 'Procedimentos', rota: '/#procedimentos' },
  { rotulo: 'Sobre nós', rota: null },  
  { rotulo: 'Contato', rota: null },   
  { rotulo: 'Agende', rota: '/agendar' },
];
...
export default function BarraDeNavegacao({
  ...
  return (
    <header
      className={`
        fixed top-0 right-0 h-14 bg-[#f5f4ec] border-b border-[#e8e6d9]
        flex items-center px-6 gap-4 z-30
        ${offsetLateral ? 'left-[152px]' : 'left-0'}
      `}
    >
      {!offsetLateral && (
        <button
          onClick={() => navigate('/')}
          className="text-xl font-black text-gray-900 tracking-tight mr-4 bg-transparent border-none cursor-pointer"
        >
          Blessed 7
        </button>
      )}
      ...
      {mostrarLinks && (
        <nav className="flex-1 flex items-center justify-center gap-8">
          {links.map(link => (
            <button
              key={link.rotulo}
              onClick={() => link.rota && navigate(link.rota)}
              className={`
                text-sm transition-colors duration-200 bg-transparent border-none cursor-pointer
                ${link.rota ? 'hover:text-[#2C3E2D]' : 'cursor-default opacity-50'}
                ${link.rotulo === 'Agende' ? 'font-bold text-[#2C3E2D]' : 'font-medium text-gray-600'}
              `}
            >
              {link.rotulo}
            </button>
      ...
      {!mostrarBusca && !mostrarLinks && <div className="flex-1" />}
      ...
      <div className="flex items-center gap-1 ml-auto">
        ...
        {mostrarNotificacoes && (
          <>
            <button className="p-2 text-gray-500 hover:text-gray-800 rounded-md hover:bg-white/60 transition-all relative">
              {iconeSino}
              <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 bg-red-500 rounded-full" />
            </button>
            <button className="p-2 text-gray-500 hover:text-gray-800 rounded-md hover:bg-white/60 transition-all">
              {iconeEngrenagem}
            </button>
          </>
        )}
...
>
  Sair
          ...
          <div className="ml-3 pl-3 border-l border-[#e0ddd0]">
            <button
              onClick={() => navigate('/login')}
              className="text-sm font-semibold text-gray-900 hover:text-[#B8982A] transition-colors duration-200 bg-transparent border-none cursor-pointer"
            >
              Entrar
            </button>
          </div>
