...
const itensNavegacao = [
  { icone: iconePainel, rotulo: 'Dashboard', ativo: false },
  { icone: iconeClientes, rotulo: 'Clientes', ativo: false },
  { icone: iconeAgendamentos, rotulo: 'Agendamentos', ativo: true },
  { icone: iconeEstoque, rotulo: 'Estoque', ativo: false },
  { icone: iconeSuporte, rotulo: 'Suporte', ativo: false },
  { icone: iconeConfiguracoes, rotulo: 'Configurações', ativo: false },
];
    ...
    <aside className="fixed top-0 left-0 h-full w-[152px] bg-[#2C3E2D] flex flex-col z-40">
      ...
      <nav className="flex-1 px-3 space-y-1">
        {itensNavegacao.map((item) => (
          ...
          >
            <span className={`flex-shrink-0 ${item.ativo ? 'text-white' : 'text-[#a8c5a0] group-hover:text-white'}`}>
              {item.icone}
            </span>
      ...
      {/* rodape */}
      <div className="px-3 pb-6 space-y-1 border-t border-white/10 pt-4 mt-4">
        <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-[#a8c5a0] hover:text-white hover:bg-white/10 transition-all text-left">
          <span>{iconeSuporte}</span>
